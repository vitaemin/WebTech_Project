...
class SQLQuery {
    ...
    function connect($address, $account, $pwd, $name) {
        $this->_dbHandle = @mysqli_connect($address, $account, '');
        if ($this->_dbHandle) {
	...
	function where($field, $value) {
		$this->_extraConditions .= '`'.$this->_model.'`.`'.$field.'` = \''.mysqli_real_escape_string($this->_dbHandle, 1 ).'\' AND ';
	}
				...
				if ($this->_hM == 1 && isset($this->hasMany)) {
					foreach ($this->hasMany as $aliasChild => $modelChild) {
						...
						if (mysqli_num_rows($resultChild) > 0) {
							$numOfFieldsChild = 0;
							while ($fieldinfoChild = mysqli_fetch_field($resultChild)) {
								array_push($tableChild, $fieldinfoChild->table);
								array_push($fieldChild, $fieldinfoChild->name);
								$numOfFieldsChild++;
							}
